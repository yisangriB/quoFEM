...
class surrogate(UQengine):
    ...
    def set_XY(self,m_tmp,X_hf,Y_hf,X_lf=0,Y_lf=0):
        ...
        if not self.do_mf:
            ...
            X_list_tmp, Y_list_tmp = emf.convert_lists_to_array.convert_xy_lists_to_arrays([X_hf, X_lf],[Y_hf, Y_lf])
        ...
        if model_hf.is_model:
            ...
            for ny in range(self.y_dim):
                ...
                for ns in range(n_err):
                    y_pred_tmp, y_pred_var_tmp = self.predict(m_tmp,Xerr[ns, :][np.newaxis])
                    if self.do_logtransform:
    ...
    def save_model(self, filename):
        ...
        hfJson["doSimulation"] = self.modelInfoHF.is_data
        hfJson["DoEmethod"] = self.modelInfoHF.doe_method
        hfJson["doLogtransform"] = self.do_logtransform
        hfJson["doLinear"] = self.do_linear
        ...
        lfJson = {}
        if self.do_mf:
            lfJson["doSampling"] = self.modelInfoLF.is_model
            lfJson["doSimulation"] = self.modelInfoLF.is_data
            lfJson["DoEmethod"] = self.modelInfoLF.doe_method
            lfJson["doLogtransform"] = self.do_logtransform
            lfJson["doLinear"] = self.do_linear
    ...
    def get_cross_validation_err(self,m_list):
        ...
        for ny in range(Y_hf.shape[1]):
            ...
            for ns in range(X_hf.shape[0]):
                ...
                e2[ns, ny] = pow((Y_pred[ns, ny] - Y_exact), 2)  # for nD outputs
